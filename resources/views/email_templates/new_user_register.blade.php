...
<title>Welcome to Drolma - New User Register!</title>
	...
	<div style="max-width: 600px; margin:0 auto;">
		<div style ="display:block; position:relative; padding:25px; background-color: #eef0f3">
			<div style="display: block; position: relative;padding: 32px;background-color: #fff;border-radius: 10px;box-shadow: 0px 0px 10px #e2e2e2;">
				...
				<p>You have received this message because your email address has been registerd with "Drolmaa" plateform. Verify yourself and confirm your email by clicking below.</p>
				<p style="text-align: center;">Clicking on the button below or use this link</p>
				<p style="text-align: center;"><a href="{{url('account/verify?email='.$emailAddress.'&userId='.$userid)}}">{{url('account/verify?email='.$emailAddress.'&userId='.$userid)}}</a></p>
				<p style="text-align: center;"><a href="{{url('account/verify?email='.$emailAddress.'&userId='.$userid)}}" style="display: inline-block;background-color: #203869;color: #fff;position: relative;border-radius: 60px;padding: 12px 30px;text-decoration: none;">Verify Email</a></p>
