...
<?php $emailAddress = base64_encode($details['email']);
	$userid = base64_encode($details['userId']);?>
	<div style="max-width: 600px; margin:0 auto;">
		<div style ="display:block; position:relative; padding:25px; background-color: #eef0f3">
			<div style="display: block; position: relative;padding: 32px;background-color: #fff;border-radius: 10px;box-shadow: 0px 0px 10px #e2e2e2;">
				...
				<p>You are receiving this mail because we got forgot password request on "Drolmaa" platform.</p>
				<p style="text-align: center;">Clicking on the button below or use this link</p>
				<p style="text-align: center;"><a href="{{url('reset_password?email='.$emailAddress.'&userId='.$userid)}}">{{url('reset_password?email='.$emailAddress.'&userId='.$userid)}}</a></p>
				<p style="text-align: center;"><a href="{{url('reset_password?email='.$emailAddress.'&userId='.$userid)}}" style="display: inline-block;background-color: #203869;color: #fff;position: relative;border-radius: 60px;padding: 12px 30px;text-decoration: none;">Reset Password</a></p>
