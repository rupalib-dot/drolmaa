            ...
            <div class="col-md-12 col-sm-12 col-lg-6 col-xl-6">
                <div class="product-name">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="col-md-2">
                            <?php 
                            $favExist = \DB::table('favourate')->where(['product_id' => $product->product_id,'user_id'=>Session::get('user_id'),'module_type'=>config('constant.WISHLIST.PRODUCTS')])->first(); ?>
                            @if(!empty($favExist))
                                <i onclick="addTofavourat('{{$product->product_id}}','{{config('constant.WISHLIST.PRODUCTS')}}','{{Session::get('user_id')}}')" style="cursor:pointer;color:#952A16;font-size: 30px;" class="fas fa-heart"></i> 
                            @else
                                <img onclick="addTofavourat('{{$product->product_id}}','{{config('constant.WISHLIST.PRODUCTS')}}','{{Session::get('user_id')}}')" style="cursor:pointer;color:#952A16;" src="{{asset('front_end/images/like.png')}}" alt="" class=" img-fluid like-img">
                            @endif    
                        </div>  
                    <div class="col-md-4">
    ...
    <div class="container">
        ...
        <div class="row" style="display: inline;" > 
            <div class="mt-5 gallery js-flickity"data-flickity-options='{ "wrapAround": true }'>
                ...
                <?php $image_name = CommonFunction::GetSingleField('product_images','image_name','product_id',$similarProduct->product_id); ?> 
                    <div class="col-sm-4"><a href="">
                        <a class="text-dark" href="{{route('page.shopDetail',$similarProduct->product_id)}}">
                            <div class="gallery-cell">
                                <div class="card border-0" style="width:18rem;">
                                    <img src="<?php if(!empty($image_name)){?>{{asset('products/'.$image_name)}}<?php }else{?>{{asset('front_end/images/blogimg.jpg')}}<?php }?>"  width="232px" height="223px" class="card-img-top" alt="...">
                                    <div class="text-center card-body">
                                        <h5 class="card-title">{{ucwords(strtolower($similarProduct->product_name))}}</h5> 
                                    </div>
                                </div>
                            </div>
                        </a>
