                                ...
                                <tbody>
                                    @if(count($record_list) > 0)
                                        @foreach($record_list as $record)
                                            <tr>
                                                <td style="max-width: 250px;">{{$record->faq_question}}</td>
                                                <td style="max-width: 550px;">{{$record->faq_answer}}</td>
                                                <td>{{date('d F, Y', strtotime($record->created_at))}}</td>
                                                <td class="text-center">
                                                    <ul class="table-controls">
                                                        <li><a href="{{route('faq.edit',base64_encode($record->faq_id))}}"  data-toggle="tooltip" data-placement="top" title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit text-primary"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a></li>
                                                        <li>
                                                            <form action="{{route('faq.destroy',base64_encode($record->faq_id))}}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
