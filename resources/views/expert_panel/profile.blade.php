                    ...
                    <div class="col-lg-10">
                        <div class="profile-form">
                            ...
                            <?php if(!empty($subscription_data)){
                                $olddate=strtotime($subscription_data->end_date); $curdate=strtotime(date('Y-m-d'));
                                if($olddate < $curdate) {?>
                                    <div style="border: 1px #952a16 solid; padding: 15px 15px 15px 15px; color: white; background: #952a16;"> 
                                        <div class="mt-3 row" <?php $olddate=strtotime($subscription_data->end_date); $curdate=strtotime(date('Y-m-d'));
                                            ...
                                            <div class="col-lg-6">
                                                <p class="text-right">Period:- {{$subscription_data->month}} Month {{$subscription_data->plan_detail}}</p>
                                        ...
                                        <form action="{{route('expert.subscribe.post')}}" method="POST">
                                            ...
                                            <script src="https://checkout.razorpay.com/v1/checkout.js"
                                                data-key="rzp_test_tazXyaYClLVzyb" data-amount="{{$record_data->register_amount}}00"
                                                data-buttontext="Renew Subscription" data-name="i4consulting.org"
                                    ...
                                    <div style="border: 1px #952a16 solid; padding: 15px 15px 15px 15px; color: white; background: #952a16;">
                                        <a style="margin-left: 10px; margin-bottom:10px;" href="{{route('page.pricing')}}"><button style="background-color: #e80808;border-color: #e80808;" class="btn btn-info" type="button">Upgrade Plan</button></a> 
                                    </div>
                               <?php }else{?>
                                    <div style="border: 1px #952a16 solid; padding: 15px 15px 15px 15px; color: white; background: #952a16;">
                                        <div class="mt-3 row" <?php $olddate=strtotime($subscription_data->end_date); $curdate=strtotime(date('Y-m-d'));
                        ...
                        <form action="{{url('expert/profile')}}/{{$record_data->user_id}}" class="formLogIn" method="POST">
                                    ...
                                    <div class="input-group mb-4">
                                        <select class="form-control" id="exampleFormControlSelect1"
                                            ...
                                            @foreach($designation_list as $desig_list)
                                                <option {{ old('designation_id', !empty($record_data) ? $record_data->designation_id : '') == $desig_list->designation_id ? 'selected' : ''}} value="{{$desig_list->designation_id}}">{{$desig_list->designation_title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="input-group mb-4">
                                        <input type="text" class="form-control" placeholder="Phone" aria-label="Phone"
                                            aria-describedby="basic-addon1" name="office_phone_number" value="{{old('office_phone_number',$record_data['office_phone_number'])}}">
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="input-group mb-4">
                                        <select class="form-control" id="exampleFormControlSelect1"
                                            ...
                                            @endphp
                                            @for($i==1;$i<=10;$i++)
                                                <option {{ old('user_experience', !empty($record_data) ? $record_data->user_experience : '') == $i ? 'selected' : ''}} value="{{$i}}">{{$i}} Year</option>
                                            @endfor
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                <textarea  name="description" class="form-control" placeholder="Describe About Yourself..." id="exampleFormControlTextarea1" rows="4">{{old('description',$record_data->description)}}</textarea>
                                </div>
                                @php
                        
                                    $special_plan =  old('special_plan', !empty($record_data) ? explode(',',$record_data->special_plan) : '');
                                @endphp
                                <div class="special">
