                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-10">
                            <div class="dashboard-panel">
                                ...
                                <a href="<?php echo e(route('expappointment.index',['type'=>'current'])); ?>"> <button class="mb-3 <?php if(!isset($request['type']) || $request['type'] == 'current'): ?> curent-appoint <?php else: ?> previous-appoint <?php endif; ?>">Current Appointment</button></a>
                                <a href="<?php echo e(route('expappointment.index',['type'=>'previous'])); ?>"> <button class="<?php if($request['type'] == 'previous'): ?> curent-appoint <?php else: ?> previous-appoint <?php endif; ?>">Previous Appointment</button></a>
                                <form action="<?php echo e(route('expappointment.index')); ?>" class="form-appoint">
                                    <div class="row">
                                    <div class="my-3 col-md-4">
                                        <input type="date" name="from_date" value="<?php echo e(old('from_date',$request['from_date'])); ?>" class="form-control mr-3">
                                    </div>
                                    <div class="my-3 col-md-4">
                                        <input  class="form-control " type="date" name="to_date" value="<?php echo e(old('to_date',$request['to_date'])); ?>">  
                                    </div>
                                    <div class="my-3 col-md-4">
                                    ...
                                    <?php if(count($appointment_list)>0): ?>
                                        <?php $__currentLoopData = $appointment_list; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $appointment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <tr>
                                                ...
                                                <td><?php echo e($appointment->users->full_name); ?></td>
                                                ...
                                                <td>
                                                    <span class="view-icon" title="Note"><a style="cursor:pointer" onclick="addNote('<?php echo e($appointment->appointment_id); ?>','<?php echo e($appointment->note); ?>')"><i class="far fa-comments"></i></a></span> 
                                                    ...
                                                    <?php if($appointment->status == config('constant.STATUS.PENDING') || $appointment->status == config('constant.STATUS.ACCEPTED')): ?>
                                                        <?php if($difference >= 48): ?>
                                                            <!-- <span onclick="return confirm('Are you sure you want to cancel this appoinment?')" class="view-icon" title="Cancel"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.CANCELLED')])); ?>"><i class="fas fa-ban"></i></a></span> -->
                                                            <span onclick="return confirm('Are you sure you want to reject this appoinment?')" class="view-icon" title="Reject"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.REJECTED')])); ?>"><i class="far fa-times-circle"></i></a></span>
                                                            <span onclick="return confirm('Are you sure you want to accept this appoinment?')" class="view-icon" title="Accept"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.ACCEPTED')])); ?>"><i class="far fa-check-square"></i></a></span>
                                                        <?php else: ?>
                                                            <?php if($appointment->status == config('constant.STATUS.PENDING')): ?>
                                                                <span onclick="return confirm('Are you sure you want to reject this appoinment?')" class="view-icon" title="Reject"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.REJECTED')])); ?>"><i class="far fa-times-circle"></i></a></span>
                                                                <span onclick="return confirm('Are you sure you want to accept this appoinment?')" class="view-icon" title="Accept"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.ACCEPTED')])); ?>"><i class="far fa-check-square"></i></a></span>
                                                            <?php elseif($appointment->status == config('constant.STATUS.ACCEPTED')): ?>
                                                                <span onclick="return confirm('Are you sure you want to complete this appoinment?')" class="view-icon" title="Complete"><a href="<?php echo e(route('expappointment.changeStatusAppoinment',['id'=>$appointment->appointment_id,'status'=>config('constant.STATUS.COMPLETED')])); ?>"><i class="far fa-check-circle"></i></a></span>
                                                            <?php else: ?>
                                                    ...
                                                    <?php elseif($appointment->status == config('constant.STATUS.COMPLETED')): ?>
                                                        ...
                                                        <?php if(empty($exist)): ?>
                                                            <span class="view-icon" title="feedback"><a style="cursor:pointer" onclick="addFeedback('<?php echo e($appointment->user_id); ?>','<?php echo e($appointment->appointment_id); ?>','<?php echo e(config("constant.FEEDBACK.APPOINMENT")); ?>')"><i class="flaticon-view-details"></i></a></span> 
                                                        <?php else: ?>
                                                            <span class="view-icon" title="feedback"><a href="<?php echo e(route('expert.feedback',['module_id'=>$appointment->appointment_id,'feedback_by'=>Session::get('user_id'),'feedback_to'=>$appointment->user_id,'module_type'=>config('constant.FEEDBACK.APPOINMENT')])); ?>"><i class="flaticon-view-details"></i></a></span> 
                                                        <?php endif; ?>
                                                    <?php else: ?>
                                                        <?php echo e(ucwords(strtolower(array_search($appointment->status,config('constant.STATUS'))))); ?>
...
<script>
    $(document).ready(function (e) { 
        ...
        $('#feedback').submit(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: "<?php echo e(url('expert/expappointment-feedback')); ?>",
                data: formData,
                cache:false,
                dataType: "json",
                contentType: false,
            ...
            <form method="POST" id="notesubmit" enctype="multipart/form-data">
                ...
                <div class="modal-body"> 
                    ...
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <div class="form-group mb-4">
                            <label for="name">Message</label>
                            <textarea class="form-control" name="description" id="description" minlength="5" maxlength="250" required rows="3" placeholder="Message"><?php echo e(old('message')); ?></textarea>
                            <span class="text-danger" id="note-error"></span>
                        </div>
                    </div> 
                </div>
...
<script>
    $(document).ready(function (e) { 
        ...
        $('#notesubmit').submit(function(e) { 
            ...
            $.ajax({
                type:'POST',
                url: "<?php echo e(route('expappointmentNote')); ?>",
                data: formData,
                cache:false,
                dataType: "json",
                contentType: false,
...
</script><?php /**PATH C:\xampp\htdocs\drolmaa\resources\views/expert_panel/appointment.blade.php ENDPATH**/ ?>