...
<script src="<?php echo e(asset('front_end/js/file-upload-with-preview.min.js')); ?>"></script>
<script src="<?php echo e(asset('front_end/js/common-script.js')); ?>"></script>
<script src="<?php echo e(asset('front_end/js/magicscroll.js')); ?>"></script>
<script>
setTimeout(function(){
    $('.error-bg').hide();
    $('.success-bg').hide();
...
function getDate(date,time_hidden = ''){
    ...
    $.ajax({
        url:"<?php echo e(route('timeslot.list.ajax')); ?>",
        type: "POST",
        data: {
            date: date,
            expert_id: expert_id,
...
{  
    $.ajax({
        url:"<?php echo e(route('workshop.detail.ajax')); ?>",
        type: "POST",
        data: {
            module_id: module_id,
            _token: '<?php echo e(csrf_token()); ?>' 
...
function state_list(country_id, state_id_hidden = '') 
{
    ...
    $('#loadingBox').removeClass('d-none');
    $.ajax({
        url:"<?php echo e(route('state.list.ajax')); ?>",
        type: "POST",
        data: {
...
function city_list(state_id, city_id_hidden = '') 
{
    ...
    $('#loadingBox').removeClass('d-none');
    $.ajax({
        url:"<?php echo e(route('city.list.ajax')); ?>",
        type: "POST",
        data: {
            state_id: state_id,
            _token: '<?php echo e(csrf_token()); ?>' 
...
<div class="loading_bg loader-block d-none" id="loadingBox">
    <div class="loading_popup">
        <div><img src="<?php echo e(asset('front_end/images/loder.gif')); ?>" width="90"></div>
        <strong class="loading_text">Processing, Please wait....</strong>
    </div>
</div><?php /**PATH J:\xampp\htdocs\drolmaa\resources\views/include/script.blade.php ENDPATH**/ ?>