...
class HomeController extends Controller
{
    ...
    { 
        ...
        $coupons = Coupons::where('start_date','<',date('Y-m-d'))->where('expiry_date','>',date('Y-m-d'))->orderBy('coupon_id','desc')->first();//coupon data
        $blogcategory = Category::paginate(3);
        $testimonial = Testimonial::get(); 
        $data   = compact('title','services','blogcategory','testimonial','category','coupons');
