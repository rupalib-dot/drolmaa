    ...
    { 
        ...
        if(count($input)  && !empty($request['razorpay_payment_id'])) 
        {
            try{
                ...
                return redirect()->route('verify.otp',['module'=>'phone','page'=>'login','user_id'=>base64_encode($user_data->user_id),'otp'=>base64_encode($otp)])->with('Success', 'Account created successfully, Check your email inbox to verify your email...');  
            } 
            catch (\Throwable $e) 
            {
