...
use App\Http\Resources\CancelReason;
...
class CommonController extends BaseController
{
    ...
    { 
		$response = Http::post('http://amantiwarii4.pythonanywhere.com/banner/user_login/7790936944');
        $quizzes = json_decode($response); 
		return $this->sendSuccess($quizzes, 'data get successfully');
	...
	public function cancelReasons()
	{
		try
		{
			$data['cancelReasons'] = array();
			$cancel_reason_data = CancelReasons::get();
		    if(count($cancel_reason_data)>0)
			{
				$data['cancelReasons'] =  CancelReason::collection($cancel_reason_data); 
			} 
			return $this->sendSuccess($data, 'Cancel Reasons listed successfully'); 
		}
		catch (\Throwable $e)
    	{
    		return $this->sendFailed($e->getMessage().' on line '.$e->getLine(), 400);  
	...
	public function contact_us(Request $request)
	{
		...
		{ 
			$data=array(
				...
				'phone' 	        => $request['phone'],
				'email' 	        => $request['email'],
				'module_type' 	    => config('contant.ENQUIERY.CONTACT'),
				'module_id' 	    => 0,
				'topic_id'          => 0,
			...
			if(!empty($count_row)){
				return $this->sendSuccess('','Contact inquiry submitted successfully');
            }else{
